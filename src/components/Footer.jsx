  ...
  return (
    <footer className="bg-navy-dark border-t border-cyan/10">
      <div className="container-max py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        {/* Col 2 */}
        <div>
          <h4 className="font-heading font-bold text-white text-lg mb-4 tracking-wide">QUICK LINKS</h4>
          <div className="flex flex-col gap-2">
            {[['/', 'Home'], ['/about', 'About Us'], ['/services', 'Services'], ['/ports', 'Ports We Serve'], ['/contact', 'Contact']].map(([path, label]) => (
              <Link key={path} to={path} className="text-white/50 hover:text-cyan text-sm font-body transition-colors">{label}</Link>
            ))}
          </div>
        </div>
        ...
        {/* Col 3 */}
        <div>
          <h4 className="font-heading font-bold text-white text-lg mb-4 tracking-wide">OUR SERVICES</h4>
          <div className="flex flex-col gap-2">
            {SERVICES.map(s => (
              <Link key={s.slug} to={`/services/${s.slug}`} className="text-white/50 hover:text-cyan text-sm font-body transition-colors">
                {s.name}
              </Link>
            ))}
          </div>
        ...
        {/* Col 4 */}
        <div>
          <h4 className="font-heading font-bold text-white text-lg mb-4 tracking-wide">CONTACT</h4>
          <div className="flex flex-col gap-3">
            ...
            <a href="mailto:info@grshipping.com" className="flex items-center gap-2 text-white/50 hover:text-cyan text-sm font-body transition-colors">
              <Mail size={14} /> info@grshipping.com
            </a>
            <a href="mailto:grshipping@hotmail.com" className="flex items-center gap-2 text-white/50 hover:text-cyan text-sm font-body transition-colors">
              <Mail size={14} /> grshipping@hotmail.com
            </a>
            <a href="https://instagram.com/gr_shipping_services" target="_blank" rel="noreferrer" className="flex items-center gap-2 text-white/50 hover:text-cyan text-sm font-body transition-colors">
          ...
          <div className="mt-6 p-3 bg-danger/10 border border-danger/30 rounded-lg">
            ...
            <a href="tel:+919652222993" className="text-white font-heading font-bold text-base">+91 96522 22993</a>
          </div>
        </div>
      </div>

      <div className="border-t border-white/5 py-5">
        <div className="container-max flex flex-col md:flex-row items-center justify-between gap-2 text-white/30 text-xs font-body">
